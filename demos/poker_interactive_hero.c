...
GameState* create_game_state(struct notcurses* nc) {
    ...
    // Initialize hero interaction state
    state->waiting_for_hero = false;
    state->current_bet = 0;
    state->hero_raise_amount = 100;
...
void draw_hero_action_menu(GameState* state) {
    ...
    // Draw menu box
    ncplane_printf_yx(n, menu_y, menu_x, "╔════════════════════════════════════════╗");
    ncplane_printf_yx(n, menu_y+1, menu_x, "║         YOUR TURN - CHOOSE ACTION      ║");
    ncplane_printf_yx(n, menu_y+2, menu_x, "╠════════════════════════════════════════╣");
    
    // Fold option
    ncplane_set_fg_rgb8(n, 255, 100, 100);
    ncplane_printf_yx(n, menu_y+3, menu_x, "║  [1] FOLD                              ║");
    
    // Call option
    ncplane_set_fg_rgb8(n, 100, 255, 100);
    if (state->current_bet > 0) {
        ncplane_printf_yx(n, menu_y+4, menu_x, "║  [2] CALL $%-4d                        ║", state->current_bet);
    } else {
        ncplane_printf_yx(n, menu_y+4, menu_x, "║  [2] CHECK                             ║");
    }
    
    // Raise option
    ncplane_set_fg_rgb8(n, 100, 150, 255);
    ncplane_printf_yx(n, menu_y+5, menu_x, "║  [3] RAISE $%-4d (↑↓ to adjust)        ║", state->hero_raise_amount);
    
    ncplane_set_fg_rgb8(n, 255, 255, 255);
    ncplane_printf_yx(n, menu_y+6, menu_x, "╚════════════════════════════════════════╝");
    
    // Highlight current bet info
    ncplane_set_fg_rgb8(n, 255, 215, 0);
    ncplane_printf_yx(n, menu_y+8, menu_x+5, "Current bet: $%d | Your chips: $%d", 
                      state->current_bet, state->players[0].chips);
}
...
void run_animation_demo(GameState* state) {
    ...
    // Betting round
    state->current_bet = 50;  // Initial bet
    ...
    for (int i = 0; i < state->num_players; i++) {
        ...
        if (i == 0) {
            ...
            while (state->waiting_for_hero) {
                ...
                // Handle input
                struct ncinput ni;
                int ret = notcurses_get_nblock(state->nc, &ni);
                ...
                if (ret > 0) {
                    if (ni.id == '1') {
                        ...
                        if (state->current_bet > 0) {
                            ...
                            player->chips -= state->current_bet;
                            state->pot += state->current_bet;
                            start_chip_animation(state, player, state->current_bet);
                            char msg[32];
                            sprintf(msg, "CALL $%d", state->current_bet);
                            show_action(state, player, msg);
                        } else {
                            show_action(state, player, "CHECK");
                        }
                        state->waiting_for_hero = false;
                    } else if (ni.id == '3') {
                        // Raise
                        player->bet = state->hero_raise_amount;
                        player->chips -= state->hero_raise_amount;
                        state->pot += state->hero_raise_amount;
                        state->current_bet = state->hero_raise_amount;
                        start_chip_animation(state, player, state->hero_raise_amount);
                        char msg[32];
                        sprintf(msg, "RAISE $%d", state->hero_raise_amount);
                        show_action(state, player, msg);
                        state->waiting_for_hero = false;
                    } else if (ni.id == NCKEY_UP) {
                        // Increase raise amount
                        state->hero_raise_amount = MIN(state->hero_raise_amount + 50, player->chips);
                    } else if (ni.id == NCKEY_DOWN) {
                        // Decrease raise amount
                        state->hero_raise_amount = MAX(state->current_bet + 50, state->hero_raise_amount - 50);
                    }
                }
...
int main(void) {
    ...
    // Print instructions
    printf("Poker Interactive Hero Demo\n");
    printf("===========================\n");
    printf("When it's your turn (bottom player):\n");
    printf("  [1] - Fold\n");
    printf("  [2] - Call/Check\n");
    printf("  [3] - Raise\n");
    printf("  ↑/↓ - Adjust raise amount\n");
